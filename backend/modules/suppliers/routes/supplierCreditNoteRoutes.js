...
// Utility function for line item validation
function validateLineItem(item) {
  return item.description && item.quantity > 0 && item.unit_price > 0 && item.amount > 0;
}
...
router.post('/', async (req, res) => {
  ...
  for (const item of lineItems) {
    if (!validateLineItem(item)) {
      return res.status(400).json({ success: false, message: 'Line item is missing required fields or contains invalid data.' });
    }
  }
