...
exports.create = async (data, userId) => {
  ...
  const [[{ nextNo }]] = await db.query('CALL get_next_cash_voucher(@no); SELECT @no AS nextNo');
  data.voucher_no = nextNo;

  // 2) write transaction in a single DB transaction
  await db.beginTransaction();
  try {
    const [res] = await db.query(
      `INSERT INTO cash_transactions
       (voucher_no, trx_type, payment_type_id, transaction_date, narration,
        reference_no, currency, amount, debit_account_id, credit_account_id,
        created_by, updated_by)
       VALUES (?,?,?,?,?,?,?,?,?,?,?,?)`,
      [
        data.voucher_no, data.trx_type, data.payment_type_id, data.transaction_date,
        data.narration, data.reference_no, data.currency, data.amount,
        data.debit_account_id, data.credit_account_id, userId, userId
      ]
    );
    await db.commit();
...
exports.findAll = async (filter = {}) => {
  let sql = `SELECT ct.*, pt.label AS payment_type,
                    dsub.sub_account_head AS debit_account,
                    csub.sub_account_head AS credit_account
             FROM cash_transactions ct
             JOIN payment_types pt      ON pt.id  = ct.payment_type_id
             JOIN sub_account_heads dsub ON dsub.id = ct.debit_account_id
             JOIN sub_account_heads csub ON csub.id = ct.credit_account_id
             WHERE 1`;
  const params = [];

  if (filter.from && filter.to) {
    sql += ` AND ct.transaction_date BETWEEN ? AND ?`;
    params.push(filter.from, filter.to);
  }
  if (filter.trx_type) {
    sql += ` AND ct.trx_type = ?`; params.push(filter.trx_type);
  }
...
exports.update = async (id, payload) => {
  const [info] = await db.query(`SELECT posted FROM cash_transactions WHERE id = ?`, [id]);
  if (!info.length) throw new Error('Transaction not found');
  if (info[0].posted) throw new Error('Posted cash voucher cannot be edited');

  await db.beginTransaction();
  try {
    await db.query(`DELETE FROM account_ledger WHERE voucher_type='Cash' AND voucher_id = ?`, [id]);
    await db.query(`
      UPDATE cash_transactions
      SET trx_type=?, payment_type_id=?, transaction_date=?, narration=?,
          reference_no=?, currency=?, amount=?, debit_account_id=?, credit_account_id=?, updated_by=?
      WHERE id = ?`,
      [
        payload.trx_type, payload.payment_type_id, payload.transaction_date,
        payload.narration, payload.reference_no, payload.currency, payload.amount,
        payload.debit_account_id, payload.credit_account_id, payload.updated_by, id
      ]
    );
    // ledger rows re-inserted automatically by AFTER UPDATE trigger
    await db.commit();
    return { id, ...payload };
  } catch (e) {
    await db.rollback();
...
exports.remove = async (id) => {
  const [info] = await db.query(`SELECT posted FROM cash_transactions WHERE id = ?`, [id]);
  if (!info.length) throw new Error('Transaction not found');
  if (info[0].posted) throw new Error('Posted cash voucher cannot be deleted');
